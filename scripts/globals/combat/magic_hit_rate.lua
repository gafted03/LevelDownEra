...
xi.combat.magicHitRate.calculateResistanceFactor = function(actor, target, skillType, actionElement, magicHitRate, rankModifier)
    ...
    switch (maxResistTier) : caseof
    {
        [1] = function()
            if randomVar <= 1 - magicHitRate then
                resistTier = 1
            end
        end,

        [2] = function()
            if randomVar <= (1 - magicHitRate) ^ 2 then
                resistTier = 2
            elseif randomVar <= (1 - magicHitRate) ^ 2 + magicHitRate * (1 - magicHitRate) then
                resistTier = 1
            end
        end,

        [3] = function()
            if randomVar <= (1 - magicHitRate) ^ 3 then
                resistTier = 3
            elseif randomVar <= (1 - magicHitRate) ^ 3 + magicHitRate * (1 - magicHitRate) ^ 2 then
                resistTier = 2
            elseif randomVar <= (1 - magicHitRate) ^ 3 + magicHitRate * (1 - magicHitRate) ^ 2 + magicHitRate * (1 - magicHitRate) then
                resistTier = 1
            end
        end,
    }
